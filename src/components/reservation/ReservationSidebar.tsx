    ...
    <div className={cn("reservation-sidebar-wrap")}>
      ...
      <InputForm title="요금">
        <div className={cn("reservation-info-box")}>
          ...
          <ReservationInfo
            left="부가세(VAT별도)"
            middle={`${((weekDayPrice * 2 + weekEndPrice * 1) * 0.1).toLocaleString()}원`}
            right={`+ ${((weekDayPrice * 2 + weekEndPrice * 1) * 0.1).toLocaleString()}원`}
          />
          <ReservationInfo
            left="연박할인"
            middle={`${diffDate >= 3 ? `10,000원 * ${(diffDate - 2).toLocaleString()}` : "-"}`}
            right={`${diffDate >= 3 ? `- ${(10000 * (diffDate - 2)).toLocaleString()}원` : "-"}`}
          />
          <div className={cn("border-line")} />
          <ReservationInfo
            left="총액"
            right={`총 ${(
              weekDayPrice * 2 +
              weekEndPrice * 1 +
              (weekDayPrice * 2 + weekEndPrice * 1) * 0.1 -
              10000 * (diffDate - 2)
            ).toLocaleString()}원`}
            isLast
          />
